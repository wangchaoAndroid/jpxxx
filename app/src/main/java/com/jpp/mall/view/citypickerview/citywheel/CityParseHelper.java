package com.jpp.mall.view.citypickerview.citywheel;
...
public class CityParseHelper {
    ...
    public void setCityBeanArrayList(List<List<CityBean>> cityBeanArrayList) {
     ...
     * 初始化数据，解析json数据
     */
    public void initData(Context context,List<CityModel.Data> cityModels) {
        //String cityJson = utils.getJson(context, Constant.CITY_DATA);
        for (int i = 0; i < cityModels.size(); i++) {
            ProvinceBean provinceBean = new ProvinceBean();
            provinceBean.setCityList(cityModels.get(i).cities);
            provinceBean.setId(cityModels.get(i).regionId);
            provinceBean.setName(cityModels.get(i).name);
            mProvinceBeanArrayList.add(provinceBean);
        }
...
//        mProvinceBeanArrayList = new Gson().fromJson(cityJson, type);
        ...
        for (int p = 0; p < mProvinceBeanArrayList.size(); p++) {
            ...
            //每个省份对应下面的市
            List<CityBean> cityList = itemProvince.getCityList();
